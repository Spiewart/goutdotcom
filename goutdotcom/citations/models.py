...
class Reference(models.Model):
    ...
    accessed = models.DateField(default=timezone.now)
    ...
    def build_citation(self):
        ...
        """
        citation = u"{0} ({1}) <i>{2}</i>".format(escape(self.author), escape(self.year), escape(self.title))

        if self.edition and self.edition > 1:
            citation += u" " + ordinal(self.edition) + u" ed"

        if self.publisher:
            citation += u". {0}{1}".format(escape(self.publisher), ": " + escape(self.place) if self.place else "")

        if self.url:
            citation += u" [Online]. Available from  <a href='{0}'>{0}</a>".format(escape(self.url))
